...
Main probability:
    p_rn    = FedRN/top-k reliable-neighbor probability from net1
...
class LocalUpdateThreeModelT1PR(LocalUpdatePFedRN):
    ...
    The goal is to separate recovery and personalization:
      p_rn    = FedRN/top-k reliable-neighbor clean probability from net1
    ...
    def train_phase2_t1pr(self, net_main, net_recovery, net_personal, self_score, neighbor_nets=None, neighbor_scores=None):
        """
        Phase2:
        ...
        # 2. Top-k neighbor fine-tuning + probability.
        # 与 original FedRN 对齐：允许使用 args.num_neighbors 个可靠邻居，
        # 而不是在三模型里强制 top-1。
        prob_list = [p_self]
...
def main():
    ...
    args.method = "pfedrn_t1pr_three"   # 新实验名：三模型 PFedRN + Top-K + Periodic Recovery
    ...
    # 不再强制 top-1 neighbor。
    # 这样命令行中的 --num_neighbors 才能与 FedRN original 对齐，例如 --num_neighbors 2。
    ...
    # 当前仍不切 holdout，保持和 FedRN / PFedRN 主结果公平。
    # 为了与 FedRN original 严格对齐，这里不做额外客户端内 shuffle。
    ...
    for user, (noise_type, noise_rate) in enumerate(user_noise_type_rates):
        ...
        # 与 FedRN original 对齐：每个客户端噪声注入前使用相同 seed。
        random.seed(args.seed)
        ...
        for edge_id, current_edge_clients in edge_clients_map.items():
            ...
            for client_idx in selected_clients:
                ...
                if epoch < args.warmup_epochs:
                    ...
                    # 选取 top-k 可靠邻居，与 original FedRN 的 --num_neighbors 对齐。
