...
# -*- coding: utf-8 -*-
"""
PFedRN-T1PR main: PFedRN personalized guidance + Top-1 reliable neighbor + Periodic Recovery.

本文件在 FedRN-T1PR 基础上接回 PFedRN 的 personalized local model guidance：
1) Top-1 reliable neighbor：只选分数最高的一个可靠邻居做 head fine-tuning；
2) PFL guidance：net2 的 local head 估计 personalized clean probability，并与 FedRN/Top-1 概率融合；
3) Periodic Recovery：从 fused probability 判为 noisy 的样本池里，用 net2 local head 周期性放回高置信样本。

模型含义：
   - net1: main/global model，上传并参与 FedAvg 聚合；
   - net2: personalized/recovery model，不上传；backbone 同步全局，local head 保留本地个性化信息。

新增统计：CleanCount / ActualTrainClean / CleanRecall / MissedClean，便于判断干净样本是否被误删。
"""
...
class LocalUpdatePFedRNT1PR(LocalUpdatePFedRN):
    """
    PFedRN personalized guidance + Top-1 neighbor + periodic recovery local update.

    net1: main/global model，上传聚合；主要使用 core clean set。
    net2: personalized/recovery model，不上传聚合；local head 用于 PFL clean probability 与 recovery。
    """
    ...
    def train_main_and_recovery(self, net_main, net_recovery, core_indices, recover_indices):
        ...
        net2/personal-recovery: 用 core + recover 训练 local head，用于 PFL clean probability 与下一轮 recovery。
        """
        ...
        # Logging only: record exactly which samples are used by the uploaded main model.
        # These indices are used later to compute ActualTrainClean / CleanRecall.
        ...
        local_ep_recovery = int(getattr(self.args, "recover_ep", getattr(self.args, "pfl_personal_ep", getattr(self.args, "local_ep", 5))))
        ...
        for _ in range(local_ep_recovery):
            ...
            for inputs, targets, items, idxs in loader_recovery:
                ...
                logits = self._forward_logits(net_recovery, inputs, head="local")
                loss = self.loss_func(logits, targets)
                loss.backward()
    ...
    def train_phase2_t1pr(self, net_main, net_recovery, self_score, neighbor_net, neighbor_score):
        """
        Phase2 (PFedRN-T1PR):
        1) self net1 GMM 得到 p_self；
        2) top-1 neighbor head fine-tuning 后得到 p_neighbor；
        3) 融合 self + top-1 neighbor 得到 p_rn；
        4) net2 local head 得到 p_local，并按 pfl_local_weight 与 p_rn 融合；
        5) 可选 global-local agreement 修正；
        6) 根据 fused probability 得到 core/noisy pool；
        7) recovery round 中，net2 从 noisy pool 放回高置信样本；
        8) net1 用 core(+recover) 训练并上传，net2 用 local head 本地训练不上传。
        """
        ...
        # 3. PFedRN personalized local clean probability from net2 local head.
        p_local = self.fit_gmm(self.net2, head="local")
        local_weight = float(getattr(self.args, "pfl_local_weight", 0.30))
        local_weight = min(max(local_weight, 0.0), 1.0)
        final_prob = (1.0 - local_weight) * p_rn + local_weight * p_local

        # 4. Optional global-local prediction agreement correction.
        ...
        # 5. Core clean set and noisy pool from fused PFedRN-T1PR probability.
        ...
        # 6. Periodic recovery from fused noisy pool.
        ...
        if do_recover:
            # net2 local head is the personalized/recovery signal.
            recover_idx = self._select_recovery_from_noisy(p_local, noisy_mask)
        else:
            recover_idx = np.array([], dtype=np.int64)

        # 7. Train two local models.
        w_main, loss_main, w_recovery, loss_recovery = self.train_main_and_recovery(
            net_main,
            net_recovery,
        ...
        train_ratio = float(len(np.unique(np.concatenate([core_idx, recover_idx])))) / float(len(self.data_indices) + 1e-8)
...
def main():
    ...
    args.method = "pfedrn_t1pr"   # 新实验名：PFedRN + Top-1 + Periodic Recovery
    args.exp_method = "PFedRN-T1PR"
    ...
    save_dir = os.path.join("resultdate", "PFedRN_T1PR")
    os.makedirs(save_dir, exist_ok=True)
    log_filename = os.path.join(
        save_dir,
        f"详细模型_PFedRN-T1PR{args.epochs}_{args.dataset}_{classes_per_user}_{args.group_noise_rate}_{timestamp}.txt",
    )
    metrics_log_filename = os.path.join(
        save_dir,
        f"总体模型_PFedRN-T1PR{args.epochs}_{args.dataset}_{classes_per_user}_{args.group_noise_rate}_{timestamp}.txt",
    )
    ...
    print("PFedRN-T1PR: PFL local guidance + top-1 neighbor + periodic recovery. Only net1 is aggregated.")
    print("PFL local weight: {}, agree weight: {}".format(args.pfl_local_weight, args.pfl_agree_weight))
    ...
    with open(metrics_log_filename, "w", encoding="utf-8") as f:
        f.write(
            ...
            "core_count,recover_count,train_count,total_count,actual_train_clean_count,"
            "actual_round_clean_count,missed_clean_count,actual_train_clean_ratio,clean_recall\n"
        )
    ...
    for epoch in range(args.epochs):
        ...
        for edge_id, current_edge_clients in edge_clients_map.items():
            ...
            for client_idx in selected_clients:
                ...
                if epoch < args.warmup_epochs:
                    ...
                    w_main, loss_main, w_recovery, loss_recovery, core_ratio, recover_ratio, train_ratio, do_recover, agreement_ratio = (
            ...
            if len(client_weights) > 0:
                ...
                edge_logs.append(
                    ...
                    "CleanCount={}/{} ActualTrainClean={:.3f} CleanRecall={:.3f} MissedClean={} trigger={:.0f}".format(
                        edge_id + 1,
                        len(client_weights),
        ...
        log_metric = (
            ...
            "Train Loss: {:.4f} | CoreClean: {:.3f} | Recover: {:.3f} | TrainRatio: {:.3f} | Agree: {:.3f} | "
            "CleanCount: {}/{} | ActualTrainClean: {:.3f} ({}/{}) | "
            "ActualCleanInRound: {} | CleanRecall: {:.3f} | MissedClean: {} | RecoveryTrigger: {:.0f}"
        ).format(
            global_test_acc,
            global_test_loss,
            aux_apa,
        ...
        with open(metrics_log_filename, "a", encoding="utf-8") as f:
            f.write(
                ...
                f"{aux_apa:.6f},{aux_loss:.6f},{core_ratio:.6f},{recover_ratio:.6f},"
                f"{train_ratio:.6f},{agree_ratio:.6f},{recovery_trigger:.0f},{round_core_count},{round_recover_count},"
                f"{round_train_count},{round_total_count},{round_actual_train_clean_count},"
                f"{round_actual_clean_count},{round_missed_clean_count},"
