...
# -*- coding: utf-8 -*-
"""
FedRN-T1PR main: Top-1 reliable neighbor + Periodic Recovery dual-model.

本文件先实现你下一阶段的“前两步”：
1) Top-1 reliable neighbor：不再用 top-k 邻居加权，先只选择分数最高的一个邻居做 head fine-tuning；
2) Recovery dual-model：本地维护两个模型：
   - net1: main/global conservative model，继承 FedRN 主干，主要维护 core clean set；
   - net2: recovery model，从 noisy pool 中周期性尝试回收可能被误删的 clean samples。

新增：周期性 recovery，默认每 5 个 global round 才从 noisy pool 放回一次样本；
非 recovery round 不放回样本，避免 clean/noisy 集合频繁震荡。

注意：这里是独立实验名 FedRN-T1PR，先不引入 PFL personalized local head 逻辑，也不加 agree。
后续如果这两步有效，再把 personalized local model 接回来。
"""
...
import time
import random
import numpy as np  # 确保 numpy 在环境修复代码的下方
# ... 其他 import
...
class LocalUpdateFedRNT1PR(LocalUpdatePFedRN):
    """
    Top-1 neighbor + recovery dual-model local update.

    net1: main/global model，上传聚合；主要使用 core clean set。
    net2: recovery model，不上传聚合；从 noisy pool 中回收部分高置信样本。
    """
    ...
    def _selection_stats(self, core_indices, recover_indices):
        """
        统计“模型认为干净的样本数量 / 原本本地样本数量”，以及真实干净比例。
        真实干净比例只用于日志分析，不参与训练。
        real_clean_mask 由 main() 在加噪前后对比得到：original_label == noisy_label。
        """
        core_indices = self._safe_indices(core_indices)
        recover_indices = self._safe_indices(recover_indices)
        train_indices = self._safe_indices(np.concatenate([core_indices, recover_indices]))
        total_count = int(len(self.data_indices))
        real_clean_mask = getattr(self, "real_clean_mask", None)

        def real_clean_count(indices):
            if real_clean_mask is None or len(indices) == 0:
                return 0
            indices = np.array(indices, dtype=np.int64)
            return int(np.sum(real_clean_mask[indices]))

        core_count = int(len(core_indices))
        recover_count = int(len(recover_indices))
        train_count = int(len(train_indices))
        actual_core_clean_count = real_clean_count(core_indices)
        actual_recover_clean_count = real_clean_count(recover_indices)
        actual_train_clean_count = real_clean_count(train_indices)
        ...
        return {
            ...
            "core_count_ratio": float(core_count) / float(total_count + 1e-8),
            "recover_count_ratio": float(recover_count) / float(total_count + 1e-8),
            "train_count_ratio": float(train_count) / float(total_count + 1e-8),
            "actual_core_clean_count": actual_core_clean_count,
            "actual_recover_clean_count": actual_recover_clean_count,
            "actual_train_clean_count": actual_train_clean_count,
            "actual_core_precision": float(actual_core_clean_count) / float(core_count + 1e-8),
            "actual_recover_precision": float(actual_recover_clean_count) / float(recover_count + 1e-8),
            "actual_train_precision": float(actual_train_clean_count) / float(train_count + 1e-8),
        }
    ...
    def _select_core_and_noisy_by_prob(self, prob, threshold):
        ...
        # 防止极端情况下 core clean set 为空：取概率最高的前 10%。
        if core_mask.sum() == 0:
    ...
    def _select_recovery_from_noisy(self, p_recover, noisy_mask):
        """
        从 p_main 判为 noisy 的样本里，用 recovery model 找回一部分。
        默认只找回 noisy pool 里 p_recover 较高的样本，并限制最大回收比例。
        """
        threshold = float(getattr(self.args, "recover_threshold", 0.60))
        max_ratio = float(getattr(self.args, "recover_max_ratio", 0.30))
        max_ratio = min(max(max_ratio, 0.0), 1.0)

        candidate_pos = np.where(noisy_mask & (p_recover > threshold))[0]
        ...
        # 只保留 p_recover 最高的一部分，避免一次性放回太多噪声。
        candidate_pos = candidate_pos[np.argsort(p_recover[candidate_pos])[::-1]]
        candidate_pos = candidate_pos[:max_recover]
        recover_idx = self.data_indices[candidate_pos]
        return self._safe_indices(recover_idx)

    def train_main_and_recovery(self, net_main, net_recovery, core_indices, recover_indices):
        """
        net1/main: 默认用 core + recover 训练，让 recovery 能对全局模型产生影响。
                   如果想更保守，把 args.recover_train_global 设为 0，则 net1 只用 core。
        net2/recovery: 用 core + recover 训练，用于下一轮继续判断 noisy pool。
        """
        recover_train_global = int(getattr(self.args, "recover_train_global", 1))
        ...
        loader_main = DataLoader(
            DatasetSplit(self.dataset, main_train_indices, real_idx_return=True),
            batch_size=self.args.local_bs,
            shuffle=True,
            num_workers=self.args.num_workers,
            pin_memory=True,
        )
        loader_recovery = DataLoader(
            DatasetSplit(self.dataset, recovery_train_indices, real_idx_return=True),
            batch_size=self.args.local_bs,
            shuffle=True,
            num_workers=self.args.num_workers,
            pin_memory=True,
        ...
        local_ep_recovery = int(getattr(self.args, "recover_ep", getattr(self.args, "local_ep", 5)))
        ...
        for _ in range(local_ep_recovery):
            ...
            for inputs, targets, items, idxs in loader_recovery:
                inputs, targets = inputs.to(self.args.device), targets.to(self.args.device)
                net_recovery.zero_grad()
                logits = self._forward_logits(net_recovery, inputs, head="global")
                loss = self.loss_func(logits, targets)
                loss.backward()
        ...
        return net_main.state_dict(), loss_main, net_recovery.state_dict(), loss_recovery

    def train_phase1_recovery(self, net_main, net_recovery):
        """Warmup: 两个模型都先使用全部本地 noisy data。"""
        w_main, loss_main, w_recovery, loss_recovery = self.train_main_and_recovery(
            net_main,
            net_recovery,
        ...
        return w_main, loss_main, w_recovery, loss_recovery

    def train_phase2_t1pr(self, net_main, net_recovery, self_score, neighbor_net, neighbor_score):
        """
        Phase2:
        1) self net1 GMM 初筛；
        2) top-1 neighbor head fine-tuning；
        3) self + top-1 neighbor 融合得到 p_main；
        4) net2/recovery 从 p_main 的 noisy pool 中回收样本；
        5) net1 / net2 分别训练。
        """
        # 1. main model self probability.
        ...
        p_main = np.zeros(len(p_self), dtype=np.float64)
        for p, s in zip(prob_list, score_list):
            p_main += np.asarray(p) * s

        # 3. Core clean set and noisy pool from main path.
        threshold = float(getattr(self.args, "p_threshold", 0.5))
        core_idx, noisy_idx, core_mask, noisy_mask = self._select_core_and_noisy_by_prob(p_main, threshold)

        # 4. Recovery model periodically tries to put back samples from noisy pool.
        #    默认每 recover_interval 轮执行一次；其余轮不放回样本。
        ...
        if do_recover:
            p_recovery = self.fit_gmm(self.net2, head="global")
            recover_idx = self._select_recovery_from_noisy(p_recovery, noisy_mask)
        else:
            recover_idx = np.array([], dtype=np.int64)

        # 5. Train two local models.
        w_main, loss_main, w_recovery, loss_recovery = self.train_main_and_recovery(
            net_main,
            net_recovery,
        ...
        stats = self._selection_stats(core_idx, recover_idx)
        stats["recovery_trigger"] = bool(do_recover)

        return w_main, loss_main, w_recovery, loss_recovery, stats


# =============================================================================
# Evaluation helper: optional holdout evaluation for net2/recovery model.
# 默认你的 dict_users_test 仍然是空，所以这个指标会是 0；后续打开 APA 时可用。
# =============================================================================
def evaluate_aux_model(local_objects, dataset, dict_users_test, args):
    ...
    for client_idx, local in enumerate(local_objects):
        ...
        with torch.no_grad():
            for images, labels in loader:
                ...
                logits = get_logits(local.net2(images), head="global")
...
def main():
    ...
    args.method = "fedrn_t1pr"   # 新实验名，避免和 pfedrn / fedco / feder 日志混淆
    args.exp_method = "FedRN-T1PR"
    ...
    save_dir = os.path.join("resultdate", "FedRN_T1PR")
    os.makedirs(save_dir, exist_ok=True)
    log_filename = os.path.join(
        save_dir,
        f"详细模型_FedRN-T1PR{args.epochs}_{args.dataset}_{classes_per_user}_{args.group_noise_rate}_{timestamp}.txt",
    )
    metrics_log_filename = os.path.join(
        save_dir,
        f"总体模型_FedRN-T1PR{args.epochs}_{args.dataset}_{classes_per_user}_{args.group_noise_rate}_{timestamp}.txt",
    )
    ...
    # 保存加噪前标签，仅用于日志中统计“实际干净样本比例”，不参与训练。
    ...
    print("Actual clean labels in noisy train set: {}/{} ({:.3f})".format(
        int(np.sum(real_clean_mask)), len(real_clean_mask), float(np.mean(real_clean_mask))
    ))
    ...
    for i in range(args.num_users):
        ...
        # 仅用于日志统计：判断被模型选中的样本里真实干净样本占比。
        local.real_clean_mask = real_clean_mask
    ...
    print("FedRN-T1PR: top-1 neighbor + periodic recovery dual-model. Only net1 is aggregated.")
    ...
    with open(metrics_log_filename, "w", encoding="utf-8") as f:
        f.write(
            "epoch,train_loss,global_test_acc,global_test_loss,aux_apa,aux_loss,"
            "core_clean_ratio,recover_ratio,train_ratio,recovery_trigger,"
            "core_count,total_count,recover_count,train_count,"
            "actual_core_precision,actual_recover_precision,actual_train_precision,"
    ...
    for epoch in range(args.epochs):
        ...
        edge_logs = []
        round_losses, round_core, round_recover, round_train_ratio, round_recovery_trigger = [], [], [], [], []
        round_core_count, round_recover_count, round_train_count, round_total_count = [], [], [], []
        round_actual_core_precision, round_actual_recover_precision, round_actual_train_precision = [], [], []
        ...
        for edge_id, current_edge_clients in edge_clients_map.items():
            ...
            client_losses, client_core, client_recover, client_train_ratio, client_recovery_trigger = [], [], [], [], []
            client_core_count, client_recover_count, client_train_count, client_total_count = [], [], [], []
            client_actual_core_precision, client_actual_recover_precision, client_actual_train_precision = [], [], []
            client_actual_train_clean_count = []
            ...
            for client_idx in selected_clients:
                ...
                if epoch < args.warmup_epochs:
                    w_main, loss_main, w_recovery, loss_recovery = local.train_phase1_recovery(
                        net_main,
                        net_recovery,
                    )
                    stats = local._selection_stats(local.data_indices, np.array([], dtype=np.int64))
                    stats["recovery_trigger"] = False
                    core_ratio = stats["core_count_ratio"]
                    recover_ratio = stats["recover_count_ratio"]
                    train_ratio = stats["train_count_ratio"]
                    do_recover = False
                else:
                    ...
                    w_main, loss_main, w_recovery, loss_recovery, stats = (
                    ...
                    recover_ratio = stats["recover_count_ratio"]
                    train_ratio = stats["train_count_ratio"]
                    do_recover = bool(stats.get("recovery_trigger", False))
                ...
                client_core_count.append(int(stats["core_count"]))
                client_recover_count.append(int(stats["recover_count"]))
                client_train_count.append(int(stats["train_count"]))
                client_total_count.append(int(stats["total_count"]))
                client_actual_core_precision.append(float(stats["actual_core_precision"]))
                client_actual_recover_precision.append(float(stats["actual_recover_precision"]))
                client_actual_train_precision.append(float(stats["actual_train_precision"]))
                client_actual_train_clean_count.append(int(stats["actual_train_clean_count"]))
            ...
            if len(client_weights) > 0:
                ...
                edge_logs.append(
                    "  --> [Edge {:02d}] clients={} loss={:.4f} core={:.3f} recover={:.3f} train={:.3f} "
                    "CleanCount={}/{} ActualTrainClean={:.3f} trigger={:.0f}".format(
                        edge_id + 1,
                        len(client_weights),
                        ...
                        int(np.sum(client_train_count)),
                        int(np.sum(client_total_count)),
                        float(np.mean(client_actual_train_precision)),
                ...
                round_core_count.extend(client_core_count)
                round_recover_count.extend(client_recover_count)
                round_train_count.extend(client_train_count)
                round_total_count.extend(client_total_count)
                round_actual_core_precision.extend(client_actual_core_precision)
                round_actual_recover_precision.extend(client_actual_recover_precision)
                round_actual_train_precision.extend(client_actual_train_precision)
                round_actual_train_clean_count.extend(client_actual_train_clean_count)
        ...
        aux_apa, aux_loss = evaluate_aux_model(local_objects, dataset_train, dict_users_test, args)
        ...
        core_count_sum = int(np.sum(round_core_count)) if round_core_count else 0
        recover_count_sum = int(np.sum(round_recover_count)) if round_recover_count else 0
        train_count_sum = int(np.sum(round_train_count)) if round_train_count else 0
        total_count_sum = int(np.sum(round_total_count)) if round_total_count else 0
        actual_core_precision = float(np.mean(round_actual_core_precision)) if round_actual_core_precision else 0.0
        actual_recover_precision = float(np.mean(round_actual_recover_precision)) if round_actual_recover_precision else 0.0
        actual_train_precision = float(np.mean(round_actual_train_precision)) if round_actual_train_precision else 0.0
        actual_train_clean_count_sum = int(np.sum(round_actual_train_clean_count)) if round_actual_train_clean_count else 0

        log_round = "\n==================== Round {:3d} ====================".format(epoch)
        log_metric = (
            "Global Acc: {:.2f}% | Global Loss: {:.4f} | Aux APA: {:.2f}% | Aux Loss: {:.4f} | "
            "Train Loss: {:.4f} | CoreClean: {:.3f} | Recover: {:.3f} | TrainRatio: {:.3f} | "
            "CleanCount: {}/{} | ActualTrainClean: {:.3f} ({}/{}) | RecoveryTrigger: {:.0f}"
        ).format(
            global_test_acc,
            global_test_loss,
            aux_apa,
            ...
            train_count_sum,
            total_count_sum,
            actual_train_precision,
            actual_train_clean_count_sum,
            train_count_sum,
            recovery_trigger,
        )
        ...
        with open(metrics_log_filename, "a", encoding="utf-8") as f:
            f.write(
                ...
                f"{aux_apa:.6f},{aux_loss:.6f},{core_ratio:.6f},{recover_ratio:.6f},{train_ratio:.6f},{recovery_trigger:.0f},"
                f"{core_count_sum},{total_count_sum},{recover_count_sum},{train_count_sum},"
                f"{actual_core_precision:.6f},{actual_recover_precision:.6f},{actual_train_precision:.6f},"
                f"{actual_train_clean_count_sum}\n"
            )
