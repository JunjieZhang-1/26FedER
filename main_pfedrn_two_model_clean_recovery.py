...
- S_recover is temporary. It is not permanently marked as clean. Every round re-scores all samples.
- net1 trains on S_core union S_recover with normal per-sample CE weight.
- net3 trains on S_core by default, so it does not self-reinforce on its own recovered samples.
...
class LocalUpdateTwoModelCleanRecovery(LocalUpdatePFedRN):
    ...
    def _select_clean_recovery_from_noisy(self, p_rn, noisy_mask, net_main, net_personal):
        ...
        # Update memory for all local samples. Recover is temporary; memory only measures recent stability.
    ...
    def train_phase2_clean_recovery(self, net_main, net_personal, self_score, neighbor_net, neighbor_score):
        ...
        # 4. Conservative clean recovery from FedRN noisy pool.
        ...
        if do_recover:
            recover_idx = self._select_clean_recovery_from_noisy(p_rn, noisy_mask, net_main, net_personal)
        else:
            # Even if not recovering this round, softly decay memory to avoid permanent marks.
            self._ensure_recovery_memory()
...
def main():
    ...
    log_filename = os.path.join(
        save_dir,
        f"详细模型_PFedRN-2M-CCR{args.epochs}_{args.dataset}_{classes_per_user}_{args.group_noise_rate}_{timestamp}.txt",
    )
    metrics_log_filename = os.path.join(
        save_dir,
        f"总体模型_PFedRN-2M-CCR{args.epochs}_{args.dataset}_{classes_per_user}_{args.group_noise_rate}_{timestamp}.txt",
    )
    ...
    print("PFedRN-2M-CCR: net1 FedRN global + net3 personalized confirmer. Only net1 is aggregated.")
    print(
        "Recovery: interval={}, max_ratio={}, conf_main={}, conf_personal={}, min_p_rn={}, memory_threshold={}".format(
    ...
    for epoch in range(args.epochs):
        ...
        for edge_id, current_edge_clients in edge_clients_map.items():
            ...
            if len(client_weights) > 0:
                ...
                edge_logs.append(
                    ...
                    "CleanCount={}/{} ActualTrainClean={:.3f} CleanRecall={:.3f} MissedClean={} FalseClean={} TotalWrong={} "
                    "RecoverClean={}/{} RecoverFalse={} RecoverPrecision={:.3f} trigger={:.0f}".format(
                        edge_id + 1,
                        len(client_weights),
        ...
        log_metric = (
            ...
            "RecoverClean: {}/{} | RecoverFalse: {} | RecoverPrecision: {:.3f} | RecoveryTrigger: {:.0f}"
        ).format(
            global_test_acc,
            global_test_loss,
            personal_apa,
        ...
        with open(metrics_log_filename, "a", encoding="utf-8") as f:
            f.write(
                ...
                f"{round_core_count},{round_recover_count},{round_train_count},{round_total_count},"
