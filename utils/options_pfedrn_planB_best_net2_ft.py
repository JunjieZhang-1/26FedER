...
核心思路：
  ...
  - warmup后：目标客户端 A 先用 netA1 得到 scoreA
  - A自己的net2只用scoreA训练，并在A的20%干净holdout上评估
  - 在本轮选中客户端中挑选 holdout 精度最高的 best_net2
  - net2_start_epoch 之前：best_net2只训练/评估，不参与最终概率，判断仍等价于原FedRN
  - net2_start_epoch 之后：对每个目标客户端 A，原FedRN概率占0.7，best_net2在scoreA上微调后的概率占0.3
  - 根据融合概率判断 clean/noisy，然后只上传net1
...
用法示例：
  python main_pfedrn_planB_best_net2_ft.py \
    ...
    --train_ratio 0.8 --fedrn_weight 0.7 --best_net2_weight 0.3 \
...
def args_parser():
    parser = argparse.ArgumentParser(
        description="PFedRN PlanB Best Net2 Fine-tuning"
    )
    ...
    parser.add_argument('--dd_alpha', type=float, default=0.1,
    ...
    parser.add_argument('--train_ratio', type=float, default=0.8,
                        help='每个客户端本地数据用于net1训练/加噪的比例；剩余部分作为net2干净holdout测试集（默认0.8）')
    ...
    # 9. PFedRN PlanB 专属参数
    # ================================================================
    parser.add_argument('--fedrn_weight', type=float, default=0.7,
                        help='原版FedRN判断概率在最终融合中的权重（默认0.7）')
    parser.add_argument('--best_net2_weight', type=float, default=0.3,
                        help='本轮best_net2微调判断概率在最终融合中的权重（默认0.3）')
    parser.add_argument('--net2_local_ep', type=int, default=5,
                        help='warmup后，net2用net1初筛scoreA训练的本地epoch数（默认5）')
    parser.add_argument('--net2_ft_ep', type=int, default=1,
                        help='best_net2在目标scoreA上临时微调分类头的遍数；默认1，建议消融1/2/3')
    parser.add_argument('--net2_start_epoch', type=int, default=200,
                        help='best_net2开始参与最终概率融合的通信轮次；默认200，表示100轮warmup后先只训练/评估net2 100轮，到第200轮再参与融合')

    # ================================================================
    # 10. 日志与保存参数
    # ================================================================
    parser.add_argument('--exp_method', type=str, default='PFedRN-PlanB-BestNet2FT',
