...
4. recover_persistent=1 时，每次新恢复的样本会累计保存到客户端自己的 recover pool。
"""
...
def args_parser():
    ...
    parser.add_argument(
        '--exp_method',
        type=str,
        default='PFedRN-2M-CCR-MergeScore',
    ...
    # 6. FedRN main branch
    # ------------------------------------------------------------------
    parser.add_argument('--num_neighbors', type=int, default=2, help='FedRN 邻居数量；main 中按该值选择 Top-K 邻居')
    ...
    parser.add_argument('--recover_max_ratio', type=float, default=0.1,
    ...
    parser.add_argument('--recover_persistent', type=int, default=1, choices=[0, 1],
                        help='是否把每次新恢复的样本累加保存到客户端 persistent recover pool；1表示启用')
    parser.add_argument('--recover_pool_max_ratio', type=float, default=1.0,
                        help='每个客户端 persistent recover pool 的最大比例；1.0表示最多可覆盖本客户端全部样本')
    parser.add_argument('--recover_clear_on_reset', type=int, default=1, choices=[0, 1],
                        help='warmup结束重置net3时是否清空 persistent recover pool；推荐1')
    parser.add_argument('--recover_merge_into_core', type=int, default=1, choices=[0, 1],
                        help='1表示把 persistent recover pool 直接并入 score/core；此时 net1 和默认 core 模式下的 net3 都会用 score+recover 训练')
