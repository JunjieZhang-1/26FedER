...
def args_parser():
    ...
    parser.add_argument('--method', type=str, default='pfedrn_t1pr_three',
                        choices=['default', 'selfie', 'jointoptim', 'coteaching', 'coteaching+', 'dividemix', 'fedrn','feder','fedrnn','fedco','fedcoPFL','pfedrn','fedrn_t1pr','pfedrn_t1pr_three'],
    ...
    parser.add_argument('--model', type=str, default='cnn4conv', choices=['cnn4conv', 'cnn_mnist','CNN4Conv_DualHead'],
    ...
    #pfedrn
    parser.add_argument('--neighbor_scope', type=str, default='edge', choices=['edge', 'global'],
    ...
    parser.add_argument('--pfl_agree_weight', type=float, default=0.15,
    ...
    # FedRN-T1PR / Top-1 Periodic Recovery 参数
    parser.add_argument('--recover_interval', type=int, default=5,
                        help='FedRN-T1PR 中 net2 周期性回收 noisy pool 的间隔轮数')
    parser.add_argument('--recover_threshold', type=float, default=0.60,
                        help='FedRN-T1PR 中 noisy pool 样本被 net2 放回的 clean probability 阈值')
    parser.add_argument('--recover_max_ratio', type=float, default=0.30,
                        help='FedRN-T1PR 中每次最多从 noisy pool 放回的比例')
    parser.add_argument('--recover_ep', type=int, default=4,
                        help='FedRN-T1PR 中 recovery model net2 的本地训练轮数')
    parser.add_argument('--recover_train_global', type=int, default=1, choices=[0, 1],
                        help='1表示net1也使用recovered samples训练；0表示net1只用core clean set')


    # Three-model PFedRN-T1PR 参数
    parser.add_argument('--t1pr_rn_weight', type=float, default=0.60,
                        help='三模型中 FedRN/top-1 分支 p_rn 的融合权重')
    parser.add_argument('--t1pr_rec_weight', type=float, default=0.20,
                        help='三模型中 recovery peer model p_rec 的融合权重')
    parser.add_argument('--t1pr_pfl_weight', type=float, default=0.20,
                        help='三模型中 personalized local model p_local 的融合权重')
    parser.add_argument('--t1pr_mutual_weight', type=float, default=0.10,
                        help='net1 与 net2 双模型互学 KL regularization 权重')
    parser.add_argument('--t1pr_mutual_temp', type=float, default=1.0,
                        help='net1 与 net2 双模型互学 KL temperature')
    parser.add_argument('--recover_need_main_support', type=int, default=1, choices=[0, 1],
                        help='1表示 recovery 样本需要 net1/FedRN 不强烈反对')
    parser.add_argument('--recover_main_min_prob', type=float, default=0.30,
                        help='recovery 样本要求 p_rn 至少大于该值')
    parser.add_argument('--recover_need_personal_support', type=int, default=0, choices=[0, 1],
                        help='1表示 recovery 样本还需要 net3/personalized branch 支持')
    parser.add_argument('--recover_personal_min_prob', type=float, default=0.60,
                        help='recovery 样本要求 p_local 至少大于该值')
    parser.add_argument('--personal_train_mode', type=str, default='core_recover', choices=['core_recover', 'all'],
                        help='net3 个性化模型训练数据：core_recover 或 all')
    parser.add_argument('--personal_distill_weight', type=float, default=0.0,
                        help='net3 从 net1 蒸馏的 KL regularization 权重，默认关闭')
